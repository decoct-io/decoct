"""End-to-end test: full pipeline on ~125 Kubernetes fleet configs.
...
class TestK8sFleetE2E:
    """Full pipeline on ~125 K8s fleet configs -- gate test."""
    ...
    def test_discovers_types(self, pipeline_result) -> None:  # type: ignore[no-untyped-def]
        """125 configs -> multiple discovered types matching config categories."""
        type_names = sorted(pipeline_result.type_map.keys())
        assert len(type_names) >= 5  # at least 5 types from 7 config categories

    def test_total_entity_count(self, pipeline_result) -> None:  # type: ignore[no-untyped-def]
        """All 125 entities present in graph."""
        assert len(pipeline_result.graph) == 125
