"""End-to-end test: full pipeline on ~80 CDN edge PoP configs.
...
class TestCdnEdgeE2E:
    """Full pipeline on ~80 CDN edge configs — gate test."""
    ...
    def test_discovers_types(self, pipeline_result) -> None:  # type: ignore[no-untyped-def]
        """80 configs → multiple discovered types matching config categories."""
    ...
    def test_total_entity_count(self, pipeline_result) -> None:  # type: ignore[no-untyped-def]
        """All 80 entities present in graph."""
        assert len(pipeline_result.graph) == 80
