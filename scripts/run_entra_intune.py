#!/usr/bin/env python3
"""Run the entity-graph pipeline on Entra ID / Intune fixtures and dump Tier A/B/C YAML."""
...
OUT_DIR = Path("output/entra-intune")


def main() -> None:
    sources = sorted(str(f) for f in FIXTURES.glob("*.json"))
    print(f"Found {len(sources)} JSON configs")

    adapter = EntraIntuneAdapter()
    config = EntityGraphConfig()

    # Pre-parse to extract relationships (needs full UUID→displayName lookup)
    graph = EntityGraph()
    for source in sources:
        parsed = adapter.parse(source)
        adapter.extract_entities(parsed, graph)
    adapter.extract_relationships(graph)
    print(f"  Relationships extracted: {len(graph.all_relationships)}")
    ...
    for tid, entities in sorted(result.type_map.items()):
        ...
        print(f"  {tid}: {len(entities)} entities, {len(h.base_class.attrs)} base attrs, {n_cls} classes, {n_sub} subclasses")
